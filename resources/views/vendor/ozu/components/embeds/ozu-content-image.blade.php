...
<div class="mb-14 md:mb-18 -mx-7.5 md:mx-0 flex flex-col items-center">
    <figure class="md:rounded-2xl md:p-2 md:bg-neutral-50 md:border border-neutral-200"
        ...
        <img class="max-w-full object-cover max-h-100 lg:max-h-128 border-y md:border-x md:rounded-xl overflow-hidden border-neutral-200" src="{{ $media->thumbnail(1400) }}" alt="{{ $legend ?? '' }}"
            x-init="imgWidth = $width" x-resize="imgWidth = $width"
        >
        @if($legend)
